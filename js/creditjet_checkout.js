jQuery(document).ready(function($) {
    ...
    const jetCalculate = () => {
        ...
        if ($('#jet_parva').val().trim() == '' || parseFloat($('#jet_parva').val()) < jetPriceall) {
            $.ajax({
                url: prestashop.urls.base_url + 'index.php?fc=module&module=creditjet&controller=JetCalculate',
                type: 'POST',
                dataType: 'json',
                data: {
            ...
            jetShowCustomAlert('Първоначалната вноска трябва да бъде по-малка от цената на стоките!', false, 0);
        }
    }
    ...
    const jetCalculateCard = () => {
        ...
        if ($('#jet_card_parva').val().trim() == '' || parseFloat($('#jet_card_parva').val()) < jetPriceall) {
            $.ajax({
                url: prestashop.urls.base_url + 'index.php?fc=module&module=creditjet&controller=JetCalculate',
                type: 'POST',
                dataType: 'json',
                data: {
                    jet_priceall: jetPriceall.toFixed(2),
                    jet_parva: parseFloat($('#jet_card_parva').val()).toFixed(2),
                ...
                success: function(json) {
                    if (json.success === 'success') {
                        if ($('#jet_card_vnoska').length !== 0) {
                            ...
                            $('#jet_card_vnoska_input').val(json.jet_vnoska_card);
                            if ($('#jet_card_vnoska_second').length !== 0) $('#jet_card_vnoska_second').text(json.jet_vnoska_card_second);
                            $('#jet_card_priceall').text(json.jet_priceall);
                            $('#jet_card_priceall_input').val(json.jet_priceall);
                            if ($('#jet_card_priceall_second').length !== 0) $('#jet_card_priceall_second').text(json.jet_priceall_second);
                            $('#jet_card_total_credit_price').text(json.jet_total_credit_price);
                            if ($('#jet_card_total_credit_price_second').length !== 0) $('#jet_card_total_credit_price_second').text(json.jet_total_credit_price_second);
                            $('#jet_card_gpr').text(json.jet_gpr);
                            $('#jet_card_glp').text(json.jet_glp);
                            $('#jet_card_obshto').text(json.jet_obshto_card);
                            if ($('#jet_card_obshto_second').length !== 0) $('#jet_card_obshto_second').text(json.jet_obshto_card_second);
                        }
                    }
                },
                error: function(error) {
            ...
            $("#jet_alert_overlay").addClass("show");
            jetShowCustomAlert('Първоначалната вноска трябва да бъде по-малка от цената на стоките!', false, 1);
        }
    }
    
    const changeBtnJetcredit = () => {
        if ($('#jet_uslovia').is(':checked') && $('#jet_uslovia1').is(':checked') && $('#jet_egn').val() != '') {
            ...
            if (_taxa >= 20) {
                ...
                $('#jet_alert_overlay').addClass('show');
                jetShowCustomAlert('Месечната вноска трябва да надхвърля сумата от 20 лв.!', false, 0);
    ...
    const changeBtnJetcreditCard = () => {
        if ($('#jet_card_uslovia').is(':checked') && $('#jet_card_uslovia1').is(':checked') && $('#jet_card_egn').val() != '') {
            ...
            if (_taxa >= 20) {
                ...
                jetShowCustomAlert('Месечната вноска трябва да надхвърля сумата от 20 лв.!', false, 1);
    ...
    $(document.body).on('click', 'input[type="radio"][name="payment-option"]', function() {
